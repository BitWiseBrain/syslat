...
int handle_sys_enter(struct trace_event_raw_sys_enter *ctx) {
    ...
    const char *syscall_names[] = {
        ...
        "io_pgetevents","rseq","uretprobe","pidfd_send_signal","io_uring_setup","io_uring_enter",
        ...
        "cachestat","fchmodat2","map_shadow_stack","futex_wake","futex_wait","futex_requeue",
        "statmount","listmount","lsm_get_self_attr","lsm_set_self_attr","lsm_list_modules",
        "mseal","setxattrat","getxattrat","listxattrat","removexattrat","open_tree_attr"
    };

    int syscall_id = BPF_CORE_READ(ctx, id);

    if (syscall_id >= sizeof(syscall_names)/sizeof(syscall_names[0]))
        return 0;
   bpf_printk("syscall hit pid = %d, tid = %d, id = %d, call = %s\n", pid, syscall_id, syscall_names[syscall_id]);
